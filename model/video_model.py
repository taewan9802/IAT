...
class Aff_channel(nn.Module):
    ...
    def forward(self, x):
        ...
        dim = self.dim
        if self.channel_first:
            color_temp = self.color.reshape(1, dim, dim)
...
class Local_pred(nn.Module):
    ...
    def forward(self, img):
        img1 = self.relu(self.conv1(img))
        mul_temp = self.mul_blocks(img1)
        mul = mul_temp + img1
        add_temp = self.add_blocks(img1)
        add = add_temp + img1
        mul = self.mul_end(mul)
        add = self.add_end(add)
        return mul, add
...
class IAT(nn.Module):
    ...
    def forward(self, img_low):
        ...
        img_high = torch.stack([self.apply_color(img_high[i,:,:,:], color[i,:,:])**gamma[i,:] for i in range(b)], dim=0).squeeze(0)
        mn = img_high.min()
        mx = img_high.max()
        mx -= mn
        return (((img_high - mn)/mx)*255).type(torch.uint8)
...
'''
    ...
    def forward(self, img_low):
        ...
        return img_high.type(torch.float32)
'''