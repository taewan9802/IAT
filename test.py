...
def context_initialize(engine):
    ...
    context.set_binding_shape(engine.get_binding_index('input.1'), (1, 3, IMAGE_HEIGHT, int(IMAGE_WIDTH/2)))
    return context
...
def image_preprocess(image_name):
    image = np.uint8(cv2.imread(image_name))
    all_frame = image[0:IMAGE_HEIGHT, 0:IMAGE_WIDTH, 0:3]
    right = image[0:IMAGE_HEIGHT, int(IMAGE_WIDTH/2):IMAGE_WIDTH, [2,1,0]]
    image = np.expand_dims(image, axis=0) # HWC -> BHWC
    image = np.moveaxis(image, 3, 1) # BHWC -> BCHW
    image = image/255.0
    return all_frame, np.ascontiguousarray(right) # input_buffer
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--engine_file', type=str, default='5_temp5.trt')
    #parser.add_argument('--engine_file', type=str, default='6_temp6.trt')
    #parser.add_argument('--engine_file', type=str, default='7_temp7.trt')
    #parser.add_argument('--engine_file', type=str, default='1107.trt')
    parser.add_argument('--engine_file', type=str, default='infer.trt')
